...
Bio and projects share one scrollable page (`src/pages/index.astro`), not two routes. Top to bottom: a non-sticky Nav (name/wordmark left, "About / Projects / Resume" right, scrolls away with the page), then the bio section (a `size-15` circular Avatar inline with the name and title line, followed by bio paragraphs capped at `max-w-[650px]`), then a `--space-section` gap, then the projects section (a row of ProjectCards, each an image plus a title/subtitle/tag stack below it). "About" and "Projects" are same-page anchor links; "Resume" links out. The Figma wireframe (both frames) is a rough layout and spacing reference only, not a pixel handoff: composition and proportion are matched, not exact coordinates.
...
- Meta text (title line under the name, ProjectCard subtitle, Tag) uses `--color-text-secondary`, never a raw gray.
...
WCAG AA is the floor for every token pair (verified in spec 0002, not just audited after the fact). Tab order follows visual order: Nav links, then in-body links, each stopping at a visible focus-visible outline. No project content or nav item may be reachable only by mouse. Layout is a single column that scales down cleanly (the 650px bio column and the project row both need a mobile behavior since the Figma wireframe is drawn at a wide desktop size only); use Tailwind's responsive variants rather than a fixed desktop-only layout.
