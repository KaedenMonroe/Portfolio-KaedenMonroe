...
- **`## Git`** (from the git integration question): write the engineer's choice as a small block, e.g. `- integration: on` / `- branch prefix: feat/` / `- commit: per-milestone` / `- worktree: on`, or just `- integration: off`. Absent means off; `worktree` absent while `integration: on` means unset, `/develop` asks inline the first time it matters rather than assuming either way. A recorded preference, not a mirrored source of truth; `/develop` reads it to branch (or worktree) and commit, `/document` to gate the PR.
