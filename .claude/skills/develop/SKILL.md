...
**Git integration:** if the nearest `AGENTS.md` `## Git` says `integration: on`, read `flow/git.md` and follow it (branch, or isolate a new feature in its own git worktree when `worktree: on`, before building; commit as milestones land); absent or `off` → do no active git.
