...
- Window resize is handled by Embla's built-in `ResizeObserver`-driven re-init, which preserves the selected index; the caption needs no manual re-sync on resize. Variable card widths do not change this: Embla natively supports variable width slides, and the caption's position already re-derives from the active card's live `offsetLeft`/width rather than assuming a fixed size.
...
- [ ] No tier heights or width envelopes are pinned by this spec; the engineer wants to see them on the real site first. Define all three (desktop, tablet, mobile) together, live in the dev server, against the real seeded project images and the oliviercarignan.com reference, with the desktop envelope specifically tuned to the AC-12 three-card clip target (active card framed, neighbors visibly clipped). This is Build plan step 13, not a separate follow-up task; a values-only change afterward does not need a further spec update, only a mechanism change (e.g. a different source than the cover's natural aspect ratio) would.
