...
- [ ] On a mobile/touch viewport, swipe across the carousel; it settles on a card each time → AC-1 (not exercised: no touch device/emulation available this run)
...
- [x] Move the carousel (drag, wheel, click, or keyboard) → the shared caption below crossfades to the new active project's title/subtitle and slides horizontally to stay aligned under it → AC-5
...
- [ ] Tab to the carousel and arrow to a project with gallery images → the active card cycles its gallery while the region has `:focus-visible`, resets on blur → AC-7 (cycling-while-focused observed, but the blur-reset half wasn't isolated and confirmed this run)
- [ ] On a touch device, or for a project with no gallery images → only the cover image ever shows, no cycling → AC-7 (not exercised: no touch device/emulation available this run)
- [ ] With `prefers-reduced-motion: reduce` set, move the carousel and hover a card → transitions are instant, not eased → AC-8 (blocked: no OS/browser media-feature toggle available in this run's tooling; the `motion-reduce:duration-0` class and the `matchMedia` listener are present and typecheck clean, but the live behavior wasn't observed)
- [ ] At a narrow (~320-336px) viewport → cards clamp to `w-[min(18rem,78vw)]` without clipping past the edge → AC-9 (blocked: the window-resize tool did not change the actual rendered viewport this run; the clamp was confirmed structurally at full width only)
- [x] Visually confirm no prev/next buttons and no dot/progress indicator appear anywhere on the carousel → AC-10
- [x] With the project list overflowing the viewport, at rest → the adjacent card(s) are visibly, partially cut off at the viewport edge (both sides mid-list, one side at either end) → AC-12
- [x] With JavaScript disabled, reload the page → every project is still reachable by native touch/wheel/keyboard scroll on the fallback track → AC-1 (progressive enhancement) — confirmed structurally: the server-rendered HTML carries the scroll-snap fallback classes and static `tabindex`/`role` before any script runs
- [ ] With a screen reader, move the carousel and wait briefly → one announcement per resting point (not one per intermediate card during a fast flick) → AC-5 (aria-live) (blocked: see report)
...
- [ ] Resize the window after moving the carousel → the caption's horizontal position re-syncs to the (possibly re-flowed) active card → Value sourcing row 6 (not exercised this run)
- [ ] Toggle OS-level reduced-motion mid-session (not just at page load) → transitions switch live, no reload needed → Value sourcing row 7 (not exercised this run)
