...
- [ ] `/check verify` found that Inter's font request pulls both the normal and italic weight-400 files even though no page currently uses italic text; nothing in this spec's component inventory needs italic. Worth restricting the `fonts` entry in `astro.config.mjs` to the normal style only (or adding `styles: ["normal"]`/granular config per Astro's Fonts API), trimming an unnecessary font download
